            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_img or view_img:  # Add bbox to image
                        ...
                        cropobj = cv2.resize(cropobj,(224,224))     # resize image to match model's expected sizing
                        cropobj = cropobj.reshape(1,224,224,3)      # return the image with shaping that TF wants.
                        prediction = traffic_sign_classifier.predict(cropobj/255)
                        index=np.argmax(prediction)
                        label = labels_dict[str(trained_class_indices[index])]
                        plot_one_box(xyxy, im0, label=label, color=colors[int(cls)], line_thickness=1)
